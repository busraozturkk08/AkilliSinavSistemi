    ...
    try {
        ...
        const result = await pool.request().query(`
            SELECT 
                ...
                CASE 
                    WHEN p.PersonelID IS NULL THEN 'Gözetmen Atanmadı'
                    ELSE LTRIM(ISNULL(p.Unvan + ' ', '') + p.Ad + ' ' + p.Soyad)
                END AS GozetmenAdi
            FROM Sinavlar s
        ...
        res.json({ haftaBasligi: "2025-2026 Bahar Dönemi Final Sınav Programı (1 - 5 Haziran)", program: groupedData });
    } catch (err) { res.status(500).json({ hata: err.message }); }
});
